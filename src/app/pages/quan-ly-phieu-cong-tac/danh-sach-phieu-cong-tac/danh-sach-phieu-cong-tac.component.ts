import { ShopDTO, StaffDTO } from './../../_models/quan-ly-phieu-cong-tac.model';
import { LuuPhieuCongTacComponent } from "./../luu-phieu-cong-tac/luu-phieu-cong-tac.component";
...
import { SuaLoTrinhCongTacComponent } from "../chi-tiet-phieu-cong-tac/components/sua-lo-trinh-cong-tac/sua-lo-trinh-cong-tac.component";
...
export class DanhSachPhieuCongTacComponent implements OnInit, OnDestroy{
  ...
  staffId: number;
  isAdmin: any;
  isHandlingLoTrinhCongTac: boolean = false;
  isViewingDetail: boolean = false;
  selectedTabIndex: number = 0;
  typeActionVoffice: number;
  currentPage: number = 0;
  currentLoTrinhPage: number = 0;
  cindex: number;
  nIndex: number;
  isSelfSignVO: boolean = false;
  ...
  /*ngAfterViewInit() {this.dataSource.sort = th
    is.sortDetail;
    this.dataSourceLoTrinh.sort = this.sort;
  }*/
  ...
  initDataBox() {
    ...
    //PURPOSE
    let requestOptionSet = {
      functionName: "getCbOptionSet",
      method: "POST",
      params: {
        userName: this.userName,
        optionSetDTO: {
          optionSetCode: "PURPOSE",
        },
      },
    };
    this.quanLyPhieuCongTacService.getListPurposeBox(requestOptionSet, true);
  ...
  eChangeProvince(type) {
    if (type == 'from') {
      this.loTrinhControl('donViDi').setValue(this.queryLoTrinh.donViDi);
      //SHOP NOI BO
      let requestShop = {
      ...
      const rqShop = this.quanLyPhieuCongTacService.getListShopBox(requestShop, false).subscribe(res => {
        this.donViDiList.next(this.quanLyPhieuCongTacService.cbxDonViNoiBoDi.value);
      });
      this.subscriptions.push(rqShop);
    } else if (type == 'to') {
      this.loTrinhControl('donViDen').setValue(this.queryLoTrinh.donViDen);
      this.loTrinhControl('dauMoiNhanSu').setValue(this.queryLoTrinh.dauMoiNhanSu);
      //STAFF NOI BO
      let requestStaff = {
      ...
      const rqStaff = this.quanLyPhieuCongTacService.getListStaffBox(requestStaff, true).subscribe(res => {
        this.dauMoiNhanVienList.next(this.quanLyPhieuCongTacService.cbxNhanVienNoiBo.value);
      });
      ...
      const rqShop = this.quanLyPhieuCongTacService.getListShopBox(requestShop, false).subscribe(res => {
        this.donViDenList.next(this.quanLyPhieuCongTacService.cbxDonViNoiBoDen.value);
      });
      this.subscriptions.push(rqShop);
    }
  }
  ...
  eSearch(isReloadingAll) {
    ...
    const rq = this.conditionSearch().subscribe((res) => {
      ...
      if (res.errorCode == "0") {
        ...
        this.isHandlingLoTrinhCongTac = isReloadingAll ? true : false;
        this.isViewingDetail = isReloadingAll ? true : false;
      } else {
  ...
  viewLoTrinhCongTac(item) {
    ...
    this.eSearchLoTrinhCongTac(false);
    this.onLoTrinhPaginateChange(this.paginatorDetail);
    this.loTrinhControl('donViDi').valueChanges.pipe(debounceTime(800)).subscribe(event => {
    ...
    this.loTrinhControl('donViDen').valueChanges.pipe(debounceTime(800)).subscribe(event => {
      if (typeof event == 'string') {
    ...
    this.loTrinhControl('dauMoiNhanSu').valueChanges.pipe(debounceTime(800)).subscribe(event => {
      if (typeof event == 'string') {
  ...
  filterShopDi(name: string) {
    ...
    const sb = this.quanLyPhieuCongTacService.getListShopBox(requestShop, false).subscribe(res => {
      this.donViDiList.next(this.quanLyPhieuCongTacService.cbxDonViNoiBoDi.value);
    });
  ...
  filterStaff(name: string) {
    ...
    const sb = this.quanLyPhieuCongTacService.getListStaffBox(requestStaff, true).subscribe(res => {
      this.dauMoiNhanVienList.next(this.quanLyPhieuCongTacService.cbxNhanVienNoiBo.value);
    });
    this.subscriptions.push(sb);
  }

  displayFnShop(user?: ShopDTO): string | undefined {
    return user ? user.shopName : undefined;
  }

  displayFnStaff(user?: StaffDTO): string | undefined {
  ...
  onLoTrinhTabChanged($event) {
    ...
    if (this.selectedTabIndex == 0) {
      ...
      this.eHandleSignVOffice(
        this.quanLyPhieuCongTacService.selectedPhieuCongTac.signVofficeStatus
      );
    }
  }
  ...
  eSendApproveRequest(item) {
    if (!this.isEnabledSendApproveRequest(item)) return;
    ...
    modalRef.componentInstance.data = {
      ...
      message: this.translate.instant('CONFIRM.SEND_REQUEST_SIGN_JOB'),
      continue: true,
      cancel: true,
      btn: [
      ...
      (result) => {
        ...
        this.commonService
          ...
          .subscribe((res) => {
            this.isLoading$ = false;
            if (res.errorCode == "0") {
              this.toastService.success(this.translate.instant('MESSAGE.SEND_REQUEST_SIGN_SUCCESS'));
              this.eSearch(false);
            } else {
              this.toastService.error(res.description);
              this.eSearch(false);
  ...
  eSave(item, isUpdate) {
    if (isUpdate && !this.isEnabledEditJob(item)) return;
    const modalRef = this.modalService.open(LuuPhieuCongTacComponent, {
      centered: true,
      backdrop: "static",
      size: "xl",
    });
    if (isUpdate) {
      ...
      modalRef.componentInstance.jobRequestId = item.jobRequestId;
      modalRef.componentInstance.tenPhieuCongTac = item.name;
      modalRef.componentInstance.mucDichCongTac = item.purpose;
      modalRef.componentInstance.moTaChiTiet = item.description;
      modalRef.componentInstance.nguonKinhPhi = item.sourceExpense;
  ...
  eDelete(item) {
    if (!this.isEnabledDeleteJob(item)) return;
    ...
    modalRef.componentInstance.data = {
      type: "WARNING",
      title: "COMMON_MODAL.WARNING",
      message: this.translate.instant('CONFIRM.DELETE_JOB_REQUEST'),
      continue: true,
      cancel: true,
      btn: [
        { text: "CANCEL", className: "btn-outline-warning btn uppercase mx-2" },
        ...
        this.commonService
          ...
          .subscribe((res) => {
            ...
            if (res.errorCode == "0") {
              ...
              this.eSearch(false);
            } else {this.translate.instant('MESSAGE.DELETE_JOB_REQUEST_SUCCESS');
                    this.toastService.error(res.description);
                    this.eSearch(false);
            }
          });
  ...
  eSearchLoTrinhCongTac(isReloadingAll) {
    ...
    const requestTarget = {
      ...
      params: {
        ...
        jobRequestDTO: {
          jobRequestId:
            this.quanLyPhieuCongTacService.selectedPhieuCongTac.jobRequestId,
        },
      },
    };
    this.commonService
      ...
      .subscribe((res) => {
        ...
        if (res.errorCode == "0") {
          ...
          this.dataSourceLoTrinh = new MatTableDataSource(
            res.data.listJobRoutingDTO
          );
      ...
      (result) => {
        const requestTarget = {
          ...
          params: {
            ...
            jobRoutingDTO: {
              jobRequestId:
                this.quanLyPhieuCongTacService.selectedPhieuCongTac
                  .jobRequestId,
              fromShopId: this.loTrinhControl('donViDi').value.shopId,
              toShopId: this.loTrinhControl('donViDen').value.shopId,
              partnerStaffId: this.loTrinhControl('dauMoiNhanSu').value == '' ? null : this.loTrinhControl('dauMoiNhanSu').value.staffId,
  ...
  eSuaLoTrinh(item) {
    if (!this.isEnabledEditRouting(this.quanLyPhieuCongTacService.selectedPhieuCongTac)) return;

    const modalRef = this.modalService.open(SuaLoTrinhCongTacComponent, {
      centered: true,
      backdrop: "static",
      size: "xl",
    });
    ...
    modalRef.componentInstance.jobRequestId = item.jobRequestId;
    modalRef.componentInstance.code = this.quanLyPhieuCongTacService.selectedPhieuCongTac.code;
    modalRef.componentInstance.queryLoTrinh.tenLoTrinh = item.name;
    modalRef.componentInstance.queryLoTrinh.startDateLoTrinh = item.fromDate;
    modalRef.componentInstance.queryLoTrinh.endDateLoTrinh = item.toDate;
    modalRef.componentInstance.queryLoTrinh.tinhDi = item.startProvinceId;
    modalRef.componentInstance.queryLoTrinh.tinhDen = item.endProvinceId;
    modalRef.componentInstance.queryLoTrinh.donViDi = item.fromShopId;
    modalRef.componentInstance.queryLoTrinh.donViDen = item.toShopId;
    modalRef.componentInstance.queryLoTrinh.dauMoiNhanSu = item.partnerStaffId ? item.partnerStaffId : null;
    modalRef.componentInstance.queryLoTrinh.doiTac = item.partnerName;
    // modalRef.componentInstance.queryLoTrinh.ghiChu = item.note;
  ...
  eXoaLoTrinh(item) {
    ...
    modalRef.componentInstance.data = {
      type: "WARNING",
      title: "COMMON_MODAL.WARNING",
      message: this.translate.instant('CONFIRM.DELETE_ROUTE_JOB'),
      continue: true,
      cancel: true,
      btn: [
        { text: "CANCEL", className: "btn-outline-warning btn uppercase mx-2" },
      ...
      (result) => {
        ...
        this.commonService
          ...
          .subscribe((res) => {
            this.isLoading$ = false;
            if (res.errorCode == "0") {
              this.toastService.success(this.translate.instant('MESSAGE.DELETE_ROUTE_SUCCESS'));
              this.eSearchLoTrinhCongTac(false);
            } else {
              this.toastService.error(res.description);
              this.eSearchLoTrinhCongTac(false);
  ...
  initDetailDataBox() {
    ...
    let requestRouting = {
      ...
      params: {
        ...
        routingDTO: {
          startProvinceId:
            this.quanLyPhieuCongTacService.selectedLoTrinhCongTac
              .startProvinceId,
          endProvinceId:
            this.quanLyPhieuCongTacService.selectedLoTrinhCongTac.endProvinceId,
          // name: ''   //autocomplete
        },
      },
    };
    ...
    let requestOther = {
      ...
      params: {
        userName: this.userName,
        optionSetDTO: {
          optionSetCode: "OTHER_TYPE",
        },
      },
    };
    this.quanLyPhieuCongTacService.getListOtherFeeBox(requestOther, true);
  ...
  getDetailData() {
    ...
    this.quanLyPhieuCongTacService.getListDetailJobRoutingData(
      requestDetailJobRouting
    );
  }

  reloadJobAndDetailData() {
    this.eSearch(true);
    this.eSearchLoTrinhCongTac(true);
    this.isViewingDetail = true;
    this.getDetailData();
  }
  ...
  isControlInvalidDate(controlName: string): boolean {
    switch (controlName) {
      case "iValidStartDate":
        ...
        if (
          this.query.iValidStartDate == undefined ||
          this.query.iValidStartDate == null ||
          (typeof this.query.iValidStartDate == "string" &&
            this.query.iValidStartDate == "")
        ) {
          this.startDateErrorMsg = this.translate.instant(
            "VALIDATION.REQUIRED",
          ...
          if (/^[0-9]{8}$/.test(iValidStartDateStr)) {
            iValidStartDateStr = [
              iValidStartDateStr.substr(0, 2),
              iValidStartDateStr.substr(2, 2),
              iValidStartDateStr.substr(4, 4),
            ].join("/");
          ...
          this.query.iValidStartDate &&
          typeof this.query.iValidStartDate == "object"
        ) {
          //check <= current date
          let currentDate = new Date();
          let currentNgbDate = new NgbDate(
          ...
          if (
            this.query.iValidEndDate &&
            typeof this.query.iValidEndDate == "object" &&
      ...
      case "iValidEndDate":
        ...
        if (
          this.query.iValidEndDate == undefined ||
          this.query.iValidEndDate == null ||
          (typeof this.query.iValidEndDate == "string" &&
            this.query.iValidEndDate == "")
        ) {
          ...
          if (/^[0-9]{8}$/.test(iValidEndDateStr)) {
            iValidEndDateStr = [
              iValidEndDateStr.substr(0, 2),
              iValidEndDateStr.substr(2, 2),
              iValidEndDateStr.substr(4, 4),
            ].join("/");
            return !this.isValidDateForm(iValidEndDateStr);
          } else {
            this.endDateErrorMsg = this.translate.instant(
              "VALIDATION.INVALID",
              { name: this.translate.instant("DATE.TO_DATE") }
            );
            return true;
    ...
    switch (controlName) {
      case "iValidStartDate":
        ...
        if (typeof this.query.iValidStartDate == "string") {
          ...
          if (/^[0-9]{8}$/.test(iValidStartDateStr)) {
            this.query.iValidStartDate = new NgbDate(
              parseInt(iValidStartDateStr.substr(4, 4)),
      ...
      case "iValidEndDate":
        ...
        if (typeof this.query.iValidEndDate == "string") {
          ...
          if (/^[0-9]{8}$/.test(iValidEndDateStr)) {
            this.query.iValidEndDate = new NgbDate(
              parseInt(iValidEndDateStr.substr(4, 4)),
  ...
  eToggleSignVO() {
    this.isSelfSignVO = !this.isSelfSignVO;
  }

  isEnabledSendApproveRequest(item) {
    ...
    // return item && (item.status == 1 || item.status == 3);
  }
