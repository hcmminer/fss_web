...
@Component({
  selector: 'app-tab-chi-phi-khac',
  templateUrl: './tab-chi-phi-khac.component.html',
  styleUrls: ['./tab-chi-phi-khac.component.scss']
})
export class TabChiPhiKhacComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const dataEvent = this.quanLyPhieuCongTacService.reloadDetailDataEvent.subscribe(event => {
      this.dataSource = new MatTableDataSource(this.quanLyPhieuCongTacService.cbxChiPhiKhacCongTac.value);
  ...
  loadAddForm() {
    this.addForm = this.fb.group({
      loaiChiPhi: ['', [Validators.required]],
      chiPhiDuChi: ['', [Validators.required, Validators.pattern(/(^\d*[1-9]\d*$)|(^(?!0+\.00)(?=.{1,15}(\.|$))(?!0(?!\.))\d{1,3}(,\d{3})*$)/)]],
      ghiChu: ['']
    });
  }
  ...
  eSave() {
    ...
    modalRef.componentInstance.data = {
      ...
      message: this.translate.instant('CONFIRM.ADD_OTHER_COST'),
      continue: true,
      cancel: true,
      btn: [
    ...
    modalRef.result.then(
      result => {
        // add
        const requestTarget = {
          functionName: 'addJobOther',
          method: 'POST',
          params: {
            userName: this.userName,
            jobOtherDTO: {
              jobRoutingId: this.quanLyPhieuCongTacService.selectedLoTrinhCongTac.id,
              otherType: this.control('loaiChiPhi').value,
              estimateAmount: this.control('chiPhiDuChi').value.replace(/,/g, ''),
        ...
        const rq = this.commonService.callAPICommon(requestTarget as RequestApiModel).subscribe(res => {
          ...
          if (res.errorCode == '0') {
            this.toastService.success(this.translate.instant('MESSAGE.ADD_OTHER_COST_SUCCESS'));
  ...
  eDelete(item) {
    ...
    modalRef.componentInstance.data = {
      type: 'WARNING',
      title: 'COMMON_MODAL.WARNING',
      message: this.translate.instant('CONFIRM.DELETE_OTHER_COST'),
      continue: true,
      cancel: true,
      btn: [
        {text: 'CANCEL', className: 'btn-outline-warning btn uppercase mx-2'},
    ...
    modalRef.result.then(
      result => {
        ...
        const rq = this.commonService.callAPICommon(requestTarget as RequestApiModel).subscribe(res => {
          this.isLoading$ = false;
          if (res.errorCode == '0') {
            this.toastService.success(this.translate.instant('MESSAGE.DELETE_OTHER_COST_SUCCESS'));
            this.reloadDataEvent.emit();
          } else {
            this.toastService.error(res.description);
          }
