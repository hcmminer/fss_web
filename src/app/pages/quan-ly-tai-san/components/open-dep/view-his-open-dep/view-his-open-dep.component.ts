...
@Component({
  selector: 'app-view-his-open-dep',
  templateUrl: './view-his-open-dep.component.html',
  styleUrls: ['./view-his-open-dep.component.scss'],
})
export class ViewHisOpenDepComponent implements OnInit {
  data;
  currentPage = 1;
  pageSize;
  ...
  ngOnInit() {
    ...
    this.eGetDetailOpenDep();
  }
  eGetDetailOpenDep() {
    const reqTar = {
      userName: this.userName,
      searchDTO: {
        depreciationId: this.data.depreciationId,
      },
    };
    const subCall = this.globalService.globalApi(reqTar, 'get-detail-open-dep').subscribe((res) => {
      if (res.errorCode == '0') {
